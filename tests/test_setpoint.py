...
def test_the_floor_is_NOT_relaxed_to_keep_the_compressor_running(sp):
    """A cap at `return water - restart differential` was tried on 2026-07-30 and
    reverted the same hour.

    It let the setpoint sit low enough that supply fell to 15.3 against a 16.0
    limit. The hard guard then closed the valves - but only hk01 and hk02 have
    actuators, so the other eight circuits kept carrying below-dew-point water
    into the slab, condensing inside it invisibly. That is precisely the failure
    the guard exists to prevent.

    So while eight of ten circuits cannot close, the SETPOINT is the only real
    control over what reaches the slab, and it must stay above limit + spread
    even when that means the compressor will not run. A non-cooling plant is
    recoverable; a wet slab is not.
    """
    c = sp()
    for _ in range(4):
    ...
    assert floor >= 20, "the floor must not be relaxed to keep the unit running"
