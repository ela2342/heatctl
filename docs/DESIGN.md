...
| Wood stove **Lohberger AC105** (confirmed 2026-07-29 — the Baubeschreibung's "La Nordica Termorosa XXL" is a stale 2021 planning entry) | 18 kW total, split water/room-direct/cooking | Own pump + return-raising mixing valve (boiler anti-condensation) | **RAUMLUFTUNABHÄNGIG** — external combustion air, chimney stack supports it. Chosen for exactly that. **Modelling consequence: firing the stove does NOT raise `n`.** A room-air-dependent stove would pull tens of m³/h of combustion air through the envelope, i.e. a switched infiltration input on top of the thermal one; this one has no such term, so the stove is a purely thermal disturbance. Sits in the living room → its room-direct share is a large local disturbance. Water side charges the buffer. Manually fired: not schedulable, only detectable. |
